...
router.post(
  ...
  async (req, res) => {
    ...
    if (seen.rowCount) return res.json({ received: true, deduped: true });
    ...
    switch (event.type) {
        ...
        await pool.query(
          `UPDATE subscriptions
           SET status='ACTIVE',
               ...
               updated_at=now()
           WHERE id=$2`,
          [invoice.lines?.data?.[0]?.period?.end ?? Math.floor(Date.now()/1000), sub.id]
        );

        // audit record
        await pool.query(
          `INSERT INTO billing_events(user_id, stripe_event_id, event_type, stripe_invoice_id, stripe_payment_intent_id, amount_paid, currency)
           VALUES($1,$2,$3,$4,$5,$6,$7)`,
          [sub.user_id, event.id, event.type, invoice.id, invoice.payment_intent ?? null, invoice.amount_paid ?? null, invoice.currency ?? null]
        );
      ...
      case "customer.subscription.deleted": {
        const stripeSubId = event.data.object.id as string;
        await pool.query(
