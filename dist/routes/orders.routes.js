...
router.post("/", validate(createOrderSchema), async (req, res, next) => {
    ...
    try {
        ...
        // 1) Upsert user
        const userRes = await client.query(`INSERT INTO users(email) VALUES($1)
       ON CONFLICT (email) DO UPDATE SET email=EXCLUDED.email
       RETURNING id,email`, [userEmail]);
        const user = userRes.rows[0];
        // 2) Create order + items in a transaction
        await client.query("BEGIN");
        const amountTotal = items.reduce((sum, it) => sum + it.unitAmount * it.quantity, 0);
        const orderRes = await client.query(`INSERT INTO orders(user_id,status,currency,amount_total)
       VALUES($1,'REQUIRES_PAYMENT',$2,$3)
       RETURNING id,status,currency,amount_total`, [user.id, currency, amountTotal]);
        const order = orderRes.rows[0];
        for (const it of items) {
            await client.query(`INSERT INTO order_items(order_id,name,unit_amount,quantity)
         VALUES($1,$2,$3,$4)`, [order.id, it.name, it.unitAmount, it.quantity]);
        }
        ...
        await pool.query("UPDATE orders SET stripe_checkout_session_id=$1 WHERE id=$2", [session.id, order.id]);
        res.status(201).json({
            orderId: order.id,
