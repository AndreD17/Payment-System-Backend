...
router.post("/checkout", async (req, res, next) => {
    try {
        ...
        // find/create user
        const uRes = await pool.query(`INSERT INTO users(email) VALUES($1)
       ON CONFLICT(email) DO UPDATE SET email=EXCLUDED.email
       RETURNING id,email,stripe_customer_id`, [userEmail]);
        ...
        const sRes = await pool.query(`INSERT INTO subscriptions(user_id, plan_id, status)
       VALUES($1,$2,'INCOMPLETE')
       RETURNING id`, [user.id, plan.id]);
        const subId = sRes.rows[0].id;
